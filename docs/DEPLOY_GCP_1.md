...
The last line is for the bank-statement import feature (OCR + PDF rasterization) —
skip it only if you're certain you won't use that feature. `pip install` cannot
provide these; they're system-level packages the Python OCR libraries call out to.
